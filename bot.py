    ...
    if u["level"] > old_lv:
        ...
        if target:
            ...
            if member:
                ...
                if rid:
                    role = guild.get_role(int(rid)); await member.add_roles(role) if role else None
...
@client.tree.command(name="rank", description="【一般】現在のレベルと進捗を画像で表示します。")
...
@client.tree.command(name="config_threshold", description="【管理】レベルアップに必要なXP（しきい値）を設定します。")
@app_commands.checks.has_permissions(administrator=True)
async def config_threshold(interaction: discord.Interaction, amount: int):
    data = load_data(); gid = str(interaction.guild.id)
    data["config"].setdefault(gid, {}).update({"xp_threshold":amount})
    save_data(data); await interaction.response.send_message(f"✅ 必要XPを {amount} に設定しました。")

@client.tree.command(name="config_happy_hour", description="【管理】XP倍率がアップする時間帯と倍率を設定します。")
@app_commands.checks.has_permissions(administrator=True)
async def config_hh(interaction: discord.Interaction, start: int, end: int, multiplier: float, enable: bool):
    ...
    save_data(data); await interaction.response.send_message(f"✅ ハッピーアワー設定完了: {start}-{end}時 {multiplier}倍")

@client.tree.command(name="config_hh_announce", description="【管理】ハッピーアワー開始・終了時の通知設定をします。")
@app_commands.checks.has_permissions(administrator=True)
async def config_hh_ann(interaction: discord.Interaction, channel: discord.TextChannel, start_msg: str, end_msg: str):
    ...
    save_data(data); await interaction.response.send_message(f"✅ 告知先: {channel.mention}")

@client.tree.command(name="config_reward", description="【管理】特定のレベルに到達した際に付与する役職を設定します。")
@app_commands.checks.has_permissions(administrator=True)
async def config_reward(interaction: discord.Interaction, level: int, role: discord.Role):
    ...
    save_data(data); await interaction.response.send_message(f"✅ レベル {level} の報酬: {role.name}")

@client.tree.command(name="config_notify", description="【管理】レベルアップ画像を表示するチャンネルを設定します。")
@app_commands.checks.has_permissions(administrator=True)
async def config_notify(interaction: discord.Interaction, channel: discord.TextChannel):
    ...
    save_data(data); await interaction.response.send_message(f"✅ 通知先: {channel.mention}")

@client.tree.command(name="config_bonus_word", description="【管理】ボーナスXPがもらえるワードを設定します。")
@app_commands.checks.has_permissions(administrator=True)
